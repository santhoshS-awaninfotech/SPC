...
resource "aws_instance" "DiscVM" {
  ...
  tags          = merge(var.common_tags, { Name = "VM-${var.reg_code}-SPC-STG-RUNR-${upper(substr(data.aws_availability_zones.available.names[count.index], -2, 2))}-${count.index + 1}${var.use_spot ? "(SPOT)" : ""}"})
  #subnet_id     = aws_subnet.discsubnet[count.index].id
